...
export function BundledRefundForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (photos.length === 0) {
        throw new Error("최소 1장의 관련 사진을 첨부해주세요.")
      }

      const excelFormData = new FormData()
      excelFormData.append("file", excelFile)

      const excelUploadRes = await fetch("/api/upload", {
        method: "POST",
        body: excelFormData,
      })
      ...
      for (const photo of photos) {
        ...
        photoUrls.push(photoUploadData.url)
      }

      const refundData = {
        type: "bundled",
        excelFile: excelUploadData.url,
        bundledPhotos: photoUrls,
      }
      ...
      router.push("/dashboard?success=bundled")
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <ImageUpload photos={photos} onPhotosChange={setPhotos} disabled={loading} label="관련 사진" required />

      <div className="rounded-lg border bg-blue-50 p-4 dark:bg-blue-950/20">
        <p className="text-sm font-medium text-blue-900 dark:text-blue-100">묶음 제출 가이드</p>
        <ul className="mt-2 space-y-1 text-xs text-blue-800 dark:text-blue-200">
          <li>• 엑셀 파일과 관련 사진들을 함께 제출합니다.</li>
          <li>• 사진은 엑셀에 명시된 모든 환불 건들과 연관됩니다.</li>
          <li>• 사진 개수에는 제한이 없습니다.</li>
        </ul>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        <Button type="submit" disabled={loading || !excelFile || photos.length === 0} className="w-full sm:flex-1">
          {loading ? (
            <>
